...
if (NOT TARGET check-${CMAKE_WORKSPACE_NAME}-${PROJECT_NAME})
    add_custom_target(check-${CMAKE_WORKSPACE_NAME}-${PROJECT_NAME} COMMAND ${CMAKE_CTEST_COMMAND} -L ${CMAKE_WORKSPACE_NAME}-${PROJECT_NAME} --output-on-failure -C ${CMAKE_CFG_INTDIR} -j ${CTEST_PARALLEL_LEVEL} WORKING_DIRECTORY ${CMAKE_BINARY_DIR})
endif ()

if (NOT TARGET tests-${CMAKE_WORKSPACE_NAME}-${PROJECT_NAME})
    add_custom_target(tests-${CMAKE_WORKSPACE_NAME}-${PROJECT_NAME} COMMENT "Build all tests for ${CMAKE_WORKSPACE_NAME}-${PROJECT_NAME}.")
    add_dependencies(check-${CMAKE_WORKSPACE_NAME}-${PROJECT_NAME} tests-${CMAKE_WORKSPACE_NAME}-${PROJECT_NAME})
endif ()
...
function(cm_mark_as_test)
    foreach (TEST_TARGET ${ARGN})
        ...
        add_dependencies(tests-${CMAKE_WORKSPACE_NAME}-${PROJECT_NAME} ${TEST_TARGET})
    endforeach ()
...
function(cm_create_internal_targets)
    if (NOT TARGET _cm_internal_tests-${CMAKE_WORKSPACE_NAME}-${PROJECT_NAME})
        ...
        cm_mark_as_test(_cm_internal_tests-${CMAKE_WORKSPACE_NAME}-${PROJECT_NAME})
    endif ()
endfunction()
...
function(cm_test_link_libraries)
    ...
    if (BUILD_TESTS)
        ...
        target_link_libraries(_cm_internal_tests-${CMAKE_WORKSPACE_NAME}-${PROJECT_NAME} ${ARGN})
    else ()
        foreach (TARGET ${ARGN})
            if (TARGET ${TARGET})
                set_property(DIRECTORY APPEND PROPERTY CM_TEST_DEPENDENCIES ${TARGET})
                target_link_libraries(_cm_internal_tests-${CMAKE_WORKSPACE_NAME}-${PROJECT_NAME} ${TARGET})
            elseif (${TARGET} MATCHES "::")
                ...
                target_link_libraries(_cm_internal_tests-${CMAKE_WORKSPACE_NAME}-${PROJECT_NAME} $<${HAS_TARGET}:${TARGET}>)
            else ()
                set_property(DIRECTORY APPEND PROPERTY CM_TEST_DEPENDENCIES ${TARGET})
                target_link_libraries(_cm_internal_tests-${CMAKE_WORKSPACE_NAME}-${PROJECT_NAME} ${TARGET})
            endif ()
            if (BUILD_SHARED_LIBS)
                target_compile_definitions(_cm_internal_tests-${CMAKE_WORKSPACE_NAME}-${PROJECT_NAME} PRIVATE -DBOOST_TEST_DYN_LINK=1 -DBOOST_TEST_NO_AUTO_LINK=1)
            endif ()
        endforeach ()
    endif ()
endfunction()
...
function(cm_test)
    ...
    if (PARSE_NAME)
        ...
        string(MAKE_C_IDENTIFIER "${CMAKE_WORKSPACE_NAME}_${PROJECT_NAME}_${SOURCES}_test" TEST_NAME)
    endif ()
...
" CONDITION $<CONFIG:${CONFIG}>)
    ...
    set_tests_properties(${TEST_NAME} PROPERTIES LABELS ${CMAKE_WORKSPACE_NAME}-${PROJECT_NAME})
...
function(cm_test_header)
    ...
    if (PARSE_NAME)
        ...
        string(MAKE_C_IDENTIFIER "${CMAKE_WORKSPACE_NAME}_${PROJECT_NAME}_${PARSE_HEADER}_header_test" TEST_NAME)
    endif ()
